...
struct Pixel {
    double r, g, b, a;
    explicit Pixel(double grey = 0, double _a = 0);
...
struct Mapping {
    ...
    Mapping();
    explicit Mapping(const string &filename);
...
Pixel::Pixel(double grey, double _a) {
    r = g = b = grey;
    a = _a;
}
...
Mapping::Mapping() { height = width = 0; }
Mapping::Mapping(const string &filename) {
    ...
    for (int i = 0; i < image.size(); ++i) {
        switch (n) {
        case 1:image[i] = Pixel(data[i * n + 0] / 255.);
            break;
        case 2:image[i] = Pixel(data[i * n + 0] / 255., 1 - data[i * n + 1] / 255.);
            break;
        case 3:
            image[i] = Pixel(
                data[i * n + 0] / 255., data[i * n + 1] / 255.,
                data[i * n + 2] / 255.
            );
            break;
        case 4:
