...
public:
    ...
    Spectrum sample(const Ray &v, Ray &l) const;

    Spectrum evaluate(const Ray &v) const;
    Spectrum sample(const Vec3 &ref, Ray &v, double &t) const;
    Spectrum sample(Ray &vB) const;
...
Spectrum Shape::sample(const Ray &v, Ray &l) const {
    assert(abs(1 - v.d.length()) < EPS);
    double t;
    ...
    l.o = intersect;
    Vec3 n = normal(intersect);
    Spectrum s = getBxDF(intersect)->sample(n, -v.d, l.d);
    return s * abs(l.d * n);
}
...
Spectrum Shape::sample(const Vec3 &ref, Ray &v, double &t) const {
    if (!isLight()) return Spectrum(0);
    Vec3 pos;
    double pdf;
    sample(pos, pdf);
...
Spectrum Shape::sample(Ray &vB) const {
    if (!isLight()) return Spectrum(0);
    double pdf;
    sample(vB.o, pdf);
    Vec3 n = normal(vB.o);
    Spectrum s = getLight(vB.o)->sample(n, vB.d) / pdf;
    return s * abs(vB.d * n);
}
