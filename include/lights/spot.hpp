...
Spectrum SpotLight::evaluate(const Vec3 &pos, const Vec3 &i) const {
    if (i.z() < 0) return Spectrum(0); else return color * D(i);
}

Spectrum SpotLight::sample(const Vec3 &pos, Vec3 &i, double &pdf, Sampler &RNG) const {
    double alpha2 = alpha * alpha;
...
double SpotLight::evaluateImportance(const Vec3 &pos, const Vec3 &i) const {
